<script type="text/javascript">
    $(function() {
        $("#button_update").on("click", function() {
            if ($("#input_update_title").val().length == 0)
            {
               ...
               var message = "Please assign session repeat";
            ...
            $.ajax({
                dataType: 'json',
                type: "POST",
                url: '<?php echo base_url(); ?>' + "admin/applications_gympro/update_session_repeat",
                data: {
                    repeat_id: $("#input_update_id").val(),
                    repeat_title: $("#input_update_title").val(),
                },
    ...
    function open_modal_update(id) {
        $.ajax({
            dataType: 'json',
            type: "POST",
            url: '<?php echo base_url(); ?>' + "admin/applications_gympro/get_session_repeat_info",
            data: {
                repeat_id: id
            },
            success: function(data) {
                $('#input_update_id').val(data.repeat_info['id']);
                $('#input_update_title').val(data.repeat_info['title']);
