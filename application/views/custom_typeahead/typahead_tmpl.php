
            <div class="row" id="page_late_id" style="display: none; position: absolute; z-index: 1001;">;
                <div class="col-md-10">
                    <div class="search_pagelet">
                        <div class="row">
                            <div class="col-md-4">
                                <div id="users_temp">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <span class="pagelet_title">People</span>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <div id="type_ahead_user">
                                                <div id="dropdown_design_user">
                                                    <div class="row margin_style">
                                                        <div class="col-md-3">
                                                            <div class="image_set">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-9 font_style">
                                                            <div class="user_name">
                                                            </div>
                                                            <div class="home_town">
                                                            </div>
                                                            <div class="country_name">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div id="news_temp">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <span class="pagelet_title">News</span>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <div id="type_ahead_news">
                                                <div id="dropdown_design_news">
                                                    <div class="row margin_style">
                                                        <div class="col-md-3">
                                                            <img src="" class="img-responsive news_image"/>
                                                        </div>
                                                        <div class="col-md-9 font_style">
                                                            <div class="news_title">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div id="recipes_temp">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <span class="pagelet_title">Recipes</span>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <div id="type_ahead_recipes">
                                                <div id="dropdown_design_recipes">
                                                    <div class="row margin_style">
                                                        <div class="col-md-3">
                                                            <img src="" class="img-responsive recipes_image"/>
                                                        </div>
                                                        <div class="col-md-9 font_style">
                                                            <div class="recipes_title">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                                <div id="blogs_temp">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <span class="pagelet_title">Blogs</span>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <div id="type_ahead_blogs">
                                                <div id="dropdown_design_blogs">
                                                    <div class="row margin_style">
                                                        <div class="col-md-3">
                                                            <img src="" class="img-responsive blogs_image"/>
                                                        </div>
                                                        <div class="col-md-9 font_style">
                                                            <div class="blogs_title">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div id="b_users_temp">
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <span class="pagelet_title">Business Groups</span>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        <div class="col-md-12">
                                            <div id="type_ahead_b_users">
                                                <div id="dropdown_design_b_users">
                                                    <div class="row margin_style">
                                                        <div class="col-md-3">
                                                            <!--<img src="" class="img-responsive b_users_image"/>-->
                                                             <div class="b_users_image">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-9 font_style">
                                                            <div class="business_name">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                            </div>
                            <div class="col-md-4"></div>
                        </div>
                    </div>
                </div>
            </div>
      

