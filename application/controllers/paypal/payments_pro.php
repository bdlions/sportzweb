...
class Payments_pro extends CI_Controller {
    ...
    function Set_express_checkout() {
        ...
        $Payment = array(
            ...
            'itemamt' => '100', // Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
        ...
        $Item = array(
            'name' => 'Product1', // Item name. 127 char max.
            'desc' => 'Description of product1', // Item description. 127 char max.
            'amt' => '100', // Cost of item.
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            redirect('https://www.sandbox.paypal.com/webscr?cmd=_express-checkout&token='.$PayPalResult['TOKEN'],'refresh');
            //$this->load->view('paypal/test_payment', $this->data);
        }
    }

    function Get_express_checkout_details() {
        $token = $this->session->userdata('paypal_token');
        print_r('verifying token:'.$token);
        $PayPalResult = $this->paypal_pro->GetExpressCheckoutDetails($token);
        print_r('Acknowledgement of PayPalResult:'.$PayPalResult['ACK']);
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            $errors = array('Errors' => $PayPalResult['ERRORS']);
            $this->load->view('paypal_error', $errors);
        } else {
    ...
    function set_express_checkout_ptpro($schedule_id = 0) {
        $this->session->unset_userdata('paypal_token');
        $this->session->unset_userdata('paypal_payer_id');
        $this->session->unset_userdata('schedule_id');
        
        $session_info = array();
        $session_info_array = $this->gympro_library->get_session_info($schedule_id)->result_array();
        ...
        $SECFields = array(
            'token' => '', // A timestamped token, the value of which was returned by a previous SetExpressCheckout call.
            'maxamt' => '', // The expected maximum total amount the order will be, including S&H and sales tax.
            'returnurl' => base_url() . 'paypal/payments_pro/get_express_checkout_details_ptpro', // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
            'cancelurl' => base_url() . GYMPRO_HOME_PAGE_PATH, // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
            'callback' => '', // URL to which the callback request from PayPal is sent.  Must start with https:// for production.
            'callbacktimeout' => '', // An override for you to request more or less time to be able to process the callback request and response.  Acceptable range for override is 1-6 seconds.  If you specify greater than 6 PayPal will use default value of 3 seconds.
            'callbackversion' => '', // The version of the Instant Update API you're using.  The default is the current version.							
            'reqconfirmshipping' => '', // The value 1 indicates that you require that the customer's shipping address is Confirmed with PayPal.  This overrides anything in the account profile.  Possible values are 1 or 0.
        ...
        $Payments = array();
        $Payment = array(
            'amt' => $session_info['cost'], // Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
            'currencycode' => '', // A three-character currency code.  Default is USD.
            'itemamt' => $session_info['cost'], // Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
            'shippingamt' => '', // Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.
            'shipdiscamt' => '', // Shipping discount for this order, specified as a negative number.
            'insuranceoptionoffered' => '', // If true, the insurance drop-down on the PayPal review page displays the string 'Yes' and the insurance amount.  If true, the total shipping insurance for this order must be a positive number.
            'handlingamt' => '', // Total handling costs for this order.  If you specify HANDLINGAMT you mut also specify a value for ITEMAMT.
        ...
        $PaymentOrderItems = array();
        $Item = array(
            'name' => $session_info['title'], // Item name. 127 char max.
            'desc' => $session_info['location'], // Item description. 127 char max.
            'amt' => $session_info['cost'], // Cost of item.
            'number' => '', // Item number.  127 char max.
            'qty' => '', // Item qty on order.  Any positive integer.
            'taxamt' => '', // Item sales tax
            'itemurl' => '', // URL for the item.
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            $this->data['token'] = $PayPalResult['TOKEN'];
            $session_data = array(
                'paypal_token' => $PayPalResult['TOKEN'],
    ...
    function get_express_checkout_details_ptpro() {
        $token = $this->session->userdata('paypal_token');
        $PayPalResult = $this->paypal_pro->GetExpressCheckoutDetails($token);
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            $errors = array('Errors' => $PayPalResult['ERRORS']);
            ...
            //$this->data['token'] = $PayPalResult['TOKEN'];
            //$this->template->load("templates/profile_setting_tmpl",'paypal/after_checkout', $this->data );
            redirect('paypal/payments_pro/do_express_checkout_payment_ptpro','refresh');
        }
    }
    
    function do_express_checkout_payment_ptpro() {
        $token = $this->session->userdata('paypal_token');
        $payerId = $this->session->userdata('paypal_payer_id');
