...
class Admin_score_prediction_library {
    ...
    public function process_imported_match($match_data)
    {
        ...
        {
            $team_id_home = $this->admin_score_prediction_model->create_team(array('title' => $home_team_title));
        }
        if($team_id_home <= 0)
        {
            return FALSE;
        ...
        {
            $team_id_away = $this->admin_score_prediction_model->create_team(array('title' => $away_team_title));
        }
        if($team_id_away <= 0)
        {
            return FALSE;
