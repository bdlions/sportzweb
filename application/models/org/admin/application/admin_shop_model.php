...
class Admin_shop_model extends Ion_auth_model
{
    ...
    protected $app_shop_product_inform_identity_column;
    public function __construct() {
        ...
        $this->app_shop_product_inform_identity_column      = $this->config->item('app_shop_product_inform_identity_column', 'ion_auth');
    }
...
//    // Youth ----------------------------------
    ...
    public function get_all_product_inform()
    {
        return $this->db->select($this->tables['app_shop_product_info'].'.id as id,'.$this->tables['app_shop_product_info'].'.*')
                    ->from($this->tables['app_shop_product_info'])
                    ->get();
    }
    /*
     * This method will return product inform info
     * @param $id, product inform id
     * @Author Nazmul on 11th November 2014
     */
    public function get_product_inform_info($id)
    {
        $this->db->where($this->tables['app_shop_product_info'].'.id', $id);
        return $this->db->select($this->tables['app_shop_product_info'].'.id as inform_id,'.$this->tables['app_shop_product_info'].'.*')
                    ->from($this->tables['app_shop_product_info'])
                    ->get();
    }
    public function product_inform_identity_check($identity = '') {
        if(empty($identity))
        {
            return FALSE;
        }
        $this->db->where($this->app_shop_product_inform_identity_column, $identity);
        return $this->db->count_all_results($this->tables['app_shop_product_info']) > 0;
    }
    
    public function create_product_inform($additional_data)
    {
        if ( array_key_exists($this->app_shop_product_inform_identity_column, $additional_data) && $this->product_inform_identity_check($additional_data[$this->app_shop_product_inform_identity_column]) )
        {
            $this->set_error('create_product_inform_duplicate_' . $this->app_shop_product_inform_identity_column);
            return FALSE;
        }
        $additional_data['created_on'] = now();
        $additional_data = $this->_filter_data($this->tables['app_shop_product_info'], $additional_data); 
        $this->db->insert($this->tables['app_shop_product_info'], $additional_data);
        $id = $this->db->insert_id();
        $this->set_message('create_product_inform_successful');
        return (isset($id)) ? $id : FALSE;
    }
    
    public function update_product_inform($id, $additional_data)
    {
        $inform_info = $this->get_product_inform_info($id)->row();
        if (array_key_exists($this->app_shop_product_inform_identity_column, $additional_data) && $this->product_inform_identity_check($additional_data[$this->app_shop_product_inform_identity_column]) && $inform_info->{$this->app_shop_product_inform_identity_column} !== $additional_data[$this->app_shop_product_inform_identity_column])
        {
            $this->set_error('update_product_inform_duplicate_' . $this->app_shop_product_inform_identity_column);
            return FALSE;
        }
        $data = $this->_filter_data($this->tables['app_shop_product_info'], $additional_data);
        $this->db->update($this->tables['app_shop_product_info'], $data, array('id' => $id));
        if ($this->db->trans_status() === FALSE) {
            $this->set_error('update_product_inform_fail');
            return FALSE;
        }
        $this->set_message('update_product_inform_successful');
        return TRUE;
    }
    
    public function delete_product_inform($id)
    {
        if(!isset($id) || $id <= 0)
        {
            $this->set_error('delete_product_inform_fail');
            return FALSE;
        }
        $this->db->where('id', $id);
        $this->db->delete($this->tables['app_shop_product_info']);
        
        if ($this->db->affected_rows() == 0) {
            $this->set_error('delete_product_inform_fail');
            return FALSE;
        }
        $this->set_message('delete_product_inform_successful');
        return TRUE;
    }
