...
class Ion_auth_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if(array_key_exists('first_name', $additional_data))
        {
            $additional_data['first_name'] = ucwords($additional_data['first_name']);
        }
        if(array_key_exists('last_name', $additional_data))
        {
            $additional_data['last_name'] = ucwords($additional_data['last_name']);
        }
    ...
    /**
     * update_last_activity time
     *
     * @return bool
     * @author alamgir
     * */
    public function update_last_activity_time($id) {
        $this->trigger_events('update_last_activity');
    ...
    public function get_users($user_id_list = array())
    {
        ...
        $this->response = $this->db->select($this->tables['users'] . '.id as user_id,'.$this->tables['users'] . '.*,'.$this->tables['basic_profile'] . '.*, ' . $this->tables['countries'] . '.country_code')
    ...
    /*
     * This method will return user info with some specific fields
     * @param $user_id_list, list of user ids
     * @Author Nazmul on 17th June 2014
     */
    public function get_users_general_info($user_id_list)
    {
        $this->db->where_in($this->tables['users'].'.id',$user_id_list);
